...
from utils.module.global_pointer_cnn import MyLoss, GlobalPointer
...
class CAILNet(PreTrainedModel):
    def __init__(self, config, dropout_rate=0.2, class_num=108):
        ...
        self.global_pointer_cnn = GlobalPointer(hidden_size=self.config.hidden_size * 2, heads=class_num,
                                                head_size=64)
    ...
    def forward(self, input_ids, attention_mask, token_type_ids=None, labels=None, gp_masks=None, triggers=None):
        ...
        # 使用global_pointer
        if gp_masks is None:
            logits = self.global_pointer_cnn(self.dropout(hidden_state), mask=attention_mask.gt(0).long(),
                                             attention_mask=attention_mask.gt(0).long()).contiguous()
        else:
            logits = self.global_pointer_cnn(self.dropout(hidden_state), mask=gp_masks.gt(0).long(),
                                             attention_mask=attention_mask.gt(0).long()).contiguous()
        if labels is None:
