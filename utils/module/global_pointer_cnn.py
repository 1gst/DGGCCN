...
from utils.module.cnn import MaskCNN
...
class GlobalPointer(nn.Module):
    ...
    def __init__(self, hidden_size, heads, head_size, RoPE=True, max_len=512, use_bias=True, tril_mask=True,
        ...
        self.cnn = MaskCNN(class_num, class_num, kernel_size=5, depth=3)
        if self.RoPE:
            self.position_embedding = RoPEPositionEncoding(max_len, head_size)

    def forward(self, inputs, mask=None, attention_mask=None):
        ''' inputs: [..., hdsz]
            mask: [bez, seq_len], padding部分为0
        '''
        # [batchsize, 512, 108*64*2]
        sequence_output = self.dense(inputs)  # [..., heads*head_size*2]
        # torch.chunk(sequence_output, self.heads, dim=-1) 8个(batchsize, 512, 64*2)
        # [batchsize, 512, 108, 64*2]
        sequence_output = torch.stack(torch.chunk(sequence_output, self.heads, dim=-1),
                                      dim=-2)  # [..., heads, head_size*2]
        # qw:[batchsize, 512, 108, 64], kw:[batchsize, 512, 108, 64]
        qw, kw = sequence_output[..., :self.head_size], sequence_output[..., self.head_size:]  # [..., heads, head_size]
        ...
        # 计算内积
        logits = torch.einsum('bhmd,bhnd->bhmn', qw, kw)  # [btz, heads, seq_len, seq_len]
        # 添加cnn层
        pad_mask = torch.full_like(logits, 1)
        pad_mask1 = 1 - attention_mask.unsqueeze(1).unsqueeze(3)  # [btz, 1, seq_len, 1]
        pad_mask2 = 1 - attention_mask.unsqueeze(1).unsqueeze(2)  # [btz, 1, 1, seq_len]
        pad_mask = pad_mask.masked_fill(pad_mask1.bool(), value=0)
        pad_mask = pad_mask.masked_fill(pad_mask2.bool(), value=0)
        cnn_logits = self.cnn(logits, pad_mask.bool())
        logits = cnn_logits + logits
        # 排除padding
        if mask is not None:
            attention_mask1 = 1 - mask.unsqueeze(1).unsqueeze(3)  # [btz, 1, seq_len, 1]
            attention_mask2 = 1 - mask.unsqueeze(1).unsqueeze(2)  # [btz, 1, 1, seq_len]
