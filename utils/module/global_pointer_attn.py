...
class GlobalPointer(nn.Module):
    ...
    def forward(self, inputs, event_inputs, mask=None):
        ...
        qw = event_output.unsqueeze(0).repeat(sequence_output.size()[0], 1,
                                              1)  # [batch_size,num_tag,head_size]
        # qw:[batchsize, 512, 108, 64], kw:[batchsize, 512, 108, 64]
        vw, kw = sequence_output[..., :self.head_size], sequence_output[..., self.head_size:]  # [..., heads, head_size]

        # ROPE编码
        if self.RoPE:
            vw = self.position_embedding(vw)
            kw = self.position_embedding(kw)

        # 计算内积和注意力
        logits = torch.einsum('bhmd,bhnd->bhmn', vw, kw)  # [btz, heads, seq_len, seq_len]
