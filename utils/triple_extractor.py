...
class LtpParser:
    def __init__(self):
        # LTP_DIR =r"../utils/ltp_data_v3.4.0"
        LTP_DIR =r"utils/ltp_data_v3.4.0"
    ...
    def format_labelrole(self, words, postags):
        ...
        roles_dict = {}
        for role in roles:
            roles_dict[role[0]] = {arg[0]: [arg[1][0], arg[1][1]] for arg in role[1]}
        return roles_dict
...
class TripleExtractor:
    ...
    def ruler1(self, words, postags, roles_dict, role_index):
        ...
        if 'A0' in role_info.keys() and 'A1' in role_info.keys():
            s = ''.join([words[word_index] for word_index in range(role_info['A0'][0], role_info['A0'][1] + 1) if
                         postags[word_index][0] not in ['w', 'u', 'x'] and words[word_index]])
            o = ''.join([words[word_index] for word_index in range(role_info['A1'][0], role_info['A1'][1] + 1) if
                         postags[word_index][0] not in ['w', 'u', 'x'] and words[word_index]])
            if s and o:
                return '1', [s, v, o]
...
if __name__ == '__main__':
    ...
    svos = extractor.triples_main("另查明，在刑事诉讼中，被害人王某某的母亲李某香向本院提起附带民事诉讼，要求被告人李某波赔偿其精神损失20000元、误工费6000元，合计26000元")
    print('\n----- 三元组 -----\n')
    print('三元组:', svos)
